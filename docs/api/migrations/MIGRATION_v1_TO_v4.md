...
This guide helps you migrate from API version v1 to v4.
...
> **Note**: Update this section with any breaking changes between v1 and v4
...
The following features are deprecated in v4 and will be removed in a future version:
...
Update your API endpoint URLs from v1 to v4:
...
**After:**
\`\`\`
https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/package
\`\`\`
...
curl -i "https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/package?productCode=MBP" \\
  -H "Authorization: Bearer \$TOKEN" \\
  -H "x-api-key: \$API_KEY"

# Test lender endpoint
curl -i "https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/lender?lenderId=LENDER-001" \\
  -H "Authorization: Bearer \$TOKEN" \\
  -H "x-api-key: \$API_KEY"

# Test product endpoint
curl -i "https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/product?productId=PROD-001" \\
  -H "Authorization: Bearer \$TOKEN" \\
  -H "x-api-key: \$API_KEY"

# Test document endpoint
curl -i "https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/document" \\
  -H "Authorization: Bearer \$TOKEN" \\
  -H "x-api-key: \$API_KEY"
\`\`\`
...
# Should return: x-api-version: v4
\`\`\`
...
console.log('API Version:', apiVersion); // Should be: v4
\`\`\`
...
print(f'API Version: {api_version}')  # Should be: v4
\`\`\`
...
**After (v4):**
\`\`\`bash
curl "https://r8ukhidr1m.execute-api.us-east-1.amazonaws.com/v4/package?productCode=MBP" \\
  -H "Authorization: Bearer \$TOKEN" \\
  -H "x-api-key: \$API_KEY"
\`\`\`
...
All endpoints are available in v4:

| Endpoint | v1 URL | v4 URL |
|----------|---------------------|----------------|
| Package | \`/v1/package\` | \`/v4/package\` |
| Lender | \`/v1/lender\` | \`/v4/lender\` |
| Product | \`/v1/product\` | \`/v4/product\` |
| Document | \`/v1/document\` | \`/v4/document\` |

## Version Headers

v4 includes the following version headers in all responses:

| Header | Description | Example |
|--------|-------------|---------|
| \`X-API-Version\` | Current API version | \`v4\` |
...
If you encounter issues with v4, you can rollback to v1:

1. Update your endpoint URLs back to \`/v1/\`
2. Revert any code changes specific to v4
...
- **v4 Release**: TODO: Add release date
