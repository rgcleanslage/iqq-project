...
# Script to create release branches across all service repositories
# Usage: ./scripts/create-release-branches.sh <version>
# Example: ./scripts/create-release-branches.sh v1
...
create_branch() {
  ...
  if git show-ref --verify --quiet "refs/heads/$branch_name"; then
    ...
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      ...
      echo "   🗑️  Deleted existing local branch"
    else
  ...
  echo "   ✅ Branch $branch_name ready in $repo"
...
echo ""
echo "🎉 Release branch creation complete!"
echo ""
echo "📝 Next steps:"
echo "   1. Verify branches were created correctly"
echo "   2. Create GitHub Release via 'Add New API Version' workflow"
echo "   3. Proceed with Task 2: Update Terraform for stage-based versioning"
echo ""
